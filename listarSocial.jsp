<%-- 
    Document   : listar
    Created on : 09/11/2020, 11:11:57
    Author     : usuario
--%>
	...
	<script src="scripts/js/editablegrid_charts.js" ></script>
        ...
        <script>
            window.onload = function() {
                ...
                metadata.push({ name: "endereco", label: "Endereço", datatype: "string", editable: false});
                metadata.push({ name: "num", label: "Nº", datatype: "string", editable: false});
                metadata.push({ name: "bairro", label: "Bairro", datatype: "string", editable: false});
                metadata.push({ name: "cep", label: "CEP", datatype: "string", editable: false});
                metadata.push({ name: "rg", label: "RG", datatype: "string", editable: false});
                metadata.push({ name: "referencia", label: "Ponto de Referencia", datatype: "string", editable: false});
                ...
                metadata.push({ name: "renda", label: "Renda", datatype: "double(,2)", editable: false});
                ...
                data.push({id: 1, values: {"classificacao":"<%= c.getClassificacao() %>","inscricao":"<%= c.getNuminscricao() %>",
                        "nome":"<%= c.getNome() %>","endereco":"<%= c.getEndereco() %>","num":"<%= c.getNum() %>","bairro":"<%= c.getBairro() %>",
                        "cep":"<%= c.getCep() %>","rg":"<%= c.getRg() %>","referencia":"<%= c.getReferencia() %>","telfixo":"<%= c.getTelfixo() %>",
                        "telcel":"<%= c.getTelcel() %>","telrec":"<%= c.getTelrecado() %>","mae":"<%= c.getNomemae() %>","pai":"<%= c.getNomepai() %>",
                        "renda":<%= c.getRenda() %>,"notaport":<%= c.getNotaport() %>,"notamat":<%= c.getNotamat() %>,"freq":<%= c.getFreq() %>,
                        "pont":<%= c.getPontuacao() %>}});
                <%
                    }
                %>
